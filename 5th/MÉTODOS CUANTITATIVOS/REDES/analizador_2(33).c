...
int  check(unsigned  char T[],unsigned char tam, unsigned char parr, unsigned char imparr); 
	...
	void analiza(unsigned char T[]) //RECIBE EN LA LISTA SENSITIVA A TODAS Y CADA UNA DE LAS TRAMAS A ANALIZAR
{
	...
	switch (T[16] & 3)
	{
		...
		if(T[23]==1){ //CABECERA ICMP
					...
					tramaICMP[0] = T[val_inicio];
					tramaICMP[1] = T[val_inicio+1];
					chekifin = check(tramaICMP,2,T[val_inicio+2],T[val_inicio+3]);
			...
			if (T[23] == 6)  //-----------------TCP------------
		{
			...
			chekifin = check(chkTCP, 12, 0, 0);
			printf("Checksum: %.2x\n", chekifin);
			if(chekifin%2){
				printf("Trama modificada\n");
			}
			else{
				printf("Trama no modificada\n");
			}
